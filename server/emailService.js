...
/**
 * Send setup request email verification code
 */
async function sendSetupRequestVerificationEmail({ email, verificationCode }) {
  const subject = 'Verify Your Email - Fellowship Setup Request';
  const htmlContent = `
    <html>
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          <h2>Email Verification</h2>
          <p>Please verify your email to continue your Fellowship Center setup request.</p>
          <div style="background-color: #f5f5f5; padding: 20px; border-radius: 5px; text-align: center; margin: 20px 0;">
            <h1 style="margin: 0; color: #2c3e50; letter-spacing: 3px;">${verificationCode}</h1>
          </div>
          <p style="color: #666; font-size: 14px;">This code will expire in 10 minutes.</p>
  ...
  const textContent = `Email Verification\n\nUse this code to verify your email for Fellowship Center setup request: ${verificationCode}\n\nThis code expires in 10 minutes.`;
...
module.exports = {
  ...
  sendSetupRequestVerificationEmail
};
